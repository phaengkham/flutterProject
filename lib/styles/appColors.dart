import 'package:flutter/animation.dart';

class AppColors {
  static final Color textColor = Color(0xFFcc7c5);
  static final Color mainColor = Color(0xFF89dad0);
  static final Color iconColor1 = Color(0xFFffd28d);
  static final Color iconColor2 = Color(0xFFfcab88);
  static final Color iconColor3 = Color(0xFF756d54);
  static final Color paraColor = Color(0xFF8f837f);
  static final Color buttonBackgroudColor = Color(0xFFf7f6f4);
  static final Color signColor = Color(0xFFa9a29f);
  static final Color titleColor = Color(0xFF5c524f);
  static final Color mainBlack = Color(0xFF332d2b);
  static final Color yellowColor = Color(0xFFffd379);
  static final Color whiteColor = Color(0xffffffff);
  static final Color shadowColor = Color(0xFFe8e8e8);
  static final Color backIconcolor = Color(0xFFfcf4e2);
}
